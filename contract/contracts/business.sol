...
contract FedX {
    ...
    function buySomething(uint16 businessHash, address user) external payable {
    ...
    function shutdownBusiness(
        ...
        require(
            businesses[businessHash].isActive,
            "Business already shut down"
        );
    ...
    function emitRegisterBusiness() external returns (string memory) {
        ...
        return
            "Event for registering your business on Loyalty-X has been emitted";
    }
    ...
    function emitBusinessShutdown() external returns (string memory) {
        ...
        return
            "Event for terminating your business on Loyalty-X has been emitted";
    }
    ...
    function emitUpdateRewardConfig() external returns (string memory) {
        emit UpdateRewardConfigEvent(
            "User wants to update their reward config"
        );
        return
            "Event to update the reward config of your business has been emitted";
    }

    function emitClaimReward() external returns (string memory) {
        emit ClaimRewardEvent("User wants to claim their reward");
        return
            "Event to claim your reward on the given business has been emitted";
    }
}
